...
void manejadorSenial(int senial)
{
    ...
    if (senial == SIGUSR2)
    {
        cout << "Recibida señal SIGUSR2, finalizando servidor." << endl;
        finalizarUsoRecursos();
        exit(0);  // Finalizar el proceso servidor
    }
}
        ...
        *memoria = cantPreguntas;
        ...
        while(cantPreguntas > 0 && getline(archPreguntas, buffer)) // Mientras pueda leer lineas.. | Serian basicamente las rondas.
        {
            while((pos = buffer.find(',')) != std::string::npos)
            {
                ...
                if(cont == 1) { // recupera la rta. válida, que se encontraba en segunda posición de la línea
                    respuesta = cortebuffer;
                    sem_post(semServidor); // como no se le envía info. al cliente, hacemos V(SERVIDOR) para seguir la ejecución
                }
                else // si i = 0 o i != 1 => recupera las preguntas u opciones
                {
