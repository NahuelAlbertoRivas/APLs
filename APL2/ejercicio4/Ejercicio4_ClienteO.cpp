...
int main(int argc, char* argv[])
{
    ...
    while(cantPreguntas > 0)
    {
        if(sem_trywait(semCliente) == 0)
        {
            ...
            memcpy(&longitud, memoria, sizeof(size_t)); // primero recupero la cantidad de bytes a leer, respectivos a la secuencia que contiene las opciones
            //cout << "Long: " << longitud << endl;
            char* buffer = new char[longitud + 1];
            memcpy(buffer, memoria + sizeof(longitud), longitud); // recupero la secuencia de opciones
            buffer[longitud] = '\0';
            cout << buffer << endl;
            sem_post(semServidor);
            i++;
